...
const Projects = () => {
  ...
  return (
    <section id="projects" className="py-24 px-6 md:px-12 bg-transparent min-h-screen transition-colors duration-300">
      <div className="container mx-auto max-w-7xl">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-4xl md:text-5xl font-display font-bold uppercase mb-4 dark:text-white">Featured Work.</h2>
            ...
            {categories.map(cat => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${activeFilter === cat
                  ...
                  : 'bg-white dark:bg-blue-900/10 text-blue-900 dark:text-blue-300 border border-blue-100 dark:border-blue-500/20 hover:border-blue-600'
                  }`}
              >
                {cat}
              </button>
          ...
          <AnimatePresence>
            {filteredProjects.map((project, idx) => (
              <a
                ...
                key={project.id}
                className={`group bg-white dark:bg-blue-900/10 rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 block ${filteredProjects.length === 5 && idx === 4 ? 'md:col-span-2' : ''
                  }`}
              >
                <div className={`relative overflow-hidden bg-gray-50 ${filteredProjects.length === 5 && idx === 4 ? 'aspect-[21/9]' : 'aspect-video'
                  ...
                  <div className="absolute inset-0 bg-blue-900/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                    <span className="bg-white text-blue-900 px-6 py-3 rounded-full font-medium transform translate-y-4 group-hover:translate-y-0 transition-all duration-300">
                      View on Behance
                    </span>
                  </div>
                </div>
                <div className="p-6">
                  <div className="flex gap-2 mb-3">
                    {project.tech.slice(0, 3).map(t => (
                      <span key={t} className="text-xs font-medium px-2 py-1 bg-blue-50 text-blue-600 rounded-md">
                        {t}
                      </span>
                    ))}
                  ...
                  <h3 className="text-2xl font-bold font-display mb-2 group-hover:text-blue-600 dark:text-blue-100 transition-colors">{project.title}</h3>
                  <p className="text-gray-500 dark:text-blue-200/60 text-sm">{project.description}</p>
                </div>
              </a>
            ))}
          </AnimatePresence>
